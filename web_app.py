...
from utils.translation import translate_to_zh
...
class WebMessageBuffer:
    def __init__(self, session_id, api_config: Optional[Dict] = None):
...
def start_analysis():
    ...
    analysis_sessions[session_id] = {
        "config": data,
        "buffer": WebMessageBuffer(session_id, data),
        "status": "running",
    }

    # Start analysis in background
    thread = threading.Thread(target=run_analysis_background, args=(session_id, data))
    thread.daemon = True
    thread.start()
...
def run_analysis_background(session_id: str, config: Dict):
    ...
    try:
        ...
        graph = TradingAgentsGraph(
            selected_analysts=config["analysts"], debug=False, config=updated_config
        )
        ...
        buffer.add_message(
            "System",
            f"Starting analysis for {config['ticker']} on {config['analysis_date']}",
        )
        buffer.update_progress(10, "Initializing analysis...")

        # Send analysis info to frontend
        socketio.emit(
            "analysis_info_update",
            {"ticker": config["ticker"], "analysis_date": config["analysis_date"]},
            room=session_id,
        )
            ...
            if len(chunk.get("messages", [])) > 0:
                ...
                if (
                    ...
                    if (
                        ...
                        buffer.add_message(
                            "Research Manager",
                            f"Final Decision: {debate_state['judge_decision']}",
                        )
                        buffer.update_progress(
